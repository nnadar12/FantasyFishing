document.addEventListener('DOMContentLoaded', () => {
    ...
    const getRegsBtn = document.getElementById('get-regs-btn');
    const stateSelect = document.getElementById('state-select');
    const regsInfoContainer = document.getElementById('regs-info-container');
    ...
    // THE CHANGE: Selectors for the mobile menu
    ...
    const leaguesData = {
        'weekend-warriors': {
            ...
            members: ['Aadit', 'Daniel', loggedInUser, 'Gavin', 'Eric']
        }
    };
    ...
    // THE CHANGE: Logic for the mobile menu
    ...
    getRegsBtn.addEventListener('click', async () => {
        ...
        if (!stateAbbr) {
            regsInfoContainer.innerHTML = '<p style="color: red;">Please select a state.</p>';
            return;
        }

        regsInfoContainer.innerHTML = '<p>Searching for regulations...</p>';
        
        try {
            const response = await fetch(`${API_URL}/api/regulations?species=${encodeURIComponent(species)}&state=${encodeURIComponent(stateFullName)}`);
            ...
            const sourceUrl = `https://www.google.com/search?q=${encodeURIComponent(stateFullName + ' fishing regulations for ' + species)}`;
            
            regsInfoContainer.innerHTML = `
                <p>${regulations.summary}</p>
                <p>
            ...
            regsInfoContainer.innerHTML = '<p style="color: red;">Could not fetch regulations. Please check official sources.</p>';
        }
    });
    ...
    function renderLeaderboard(leagueId) {
        ...
        const leagueCatches = masterFeedData.filter(item => leagueMembers.has(item.username));

        leagueCatches.forEach(item => {
            const member = item.username;
            const stats = memberStats[member];

            stats.totalPoints += item.points;
            stats.species.add(item.species);

            const currentWeight = parseFloat(item.weight);
            const biggestSoFar = stats.biggestFish[item.species];
            
            if (!biggestSoFar || currentWeight > parseFloat(biggestSoFar.weight)) {
                stats.biggestFish[item.species] = item;
            }
        });
        ...
        const sortedByPoints = Object.entries(memberStats)
            .map(([user, stats]) => ({ user, totalPoints: stats.totalPoints }))
            .sort((a, b) => b.totalPoints - a.totalPoints);

        html += '<h2>Total Points</h2><table class="leaderboard-table"><tr><th>Rank</th><th>User</th><th>Points</th></tr>';
        sortedByPoints.forEach(({ user, totalPoints }, index) => {
            html += `<tr><td>${index + 1}</td><td>${user}</td><td>${totalPoints}</td></tr>`;
        });
        html += '</table>';

        const sortedBySpecies = Object.entries(memberStats)
            .map(([user, stats]) => ({ user, speciesCount: stats.species.size }))
            .sort((a, b) => b.speciesCount - a.speciesCount);
        
        html += '<h2 style="margin-top: 2rem;">Species Hunt</h2><table class="leaderboard-table"><tr><th>Rank</th><th>User</th><th>Unique Species</th></tr>';
        sortedBySpecies.forEach(({ user, speciesCount }, index) => {
            html += `<tr><td>${index + 1}</td><td>${user}</td><td>${speciesCount}</td></tr>`;
        });
        html += '</table>';

        const leagueBiggestFish = {};
        Object.values(memberStats).forEach(stats => {
            for (const species in stats.biggestFish) {
                const fish = stats.biggestFish[species];
                const currentWeight = parseFloat(fish.weight);
                if (!leagueBiggestFish[species] || currentWeight > parseFloat(leagueBiggestFish[species].weight)) {
                    leagueBiggestFish[species] = fish;
                }
            }
        });

        html += '<h2 style="margin-top: 2rem;">Heaviest Fish</h2>';
        const sortedSpecies = Object.keys(leagueBiggestFish).sort();

        if (sortedSpecies.length === 0) {
            html += '<p>No fish caught in this league yet!</p>';
        } else {
            html += '<div class="heaviest-fish-list">';
            sortedSpecies.forEach(species => {
                const fish = leagueBiggestFish[species];
                html += `
                    <div class="heaviest-fish-item">
                        <strong>Biggest ${species}</strong>
                        <span>${fish.weight} - ${fish.username}</span>
                    </div>
                `;
            });
            html += '</div>';
